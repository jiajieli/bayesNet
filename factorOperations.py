...
def eliminateWithCallTracking(callTrackingList=None):
    ...
    def eliminate(factor, eliminationVariable):
        ...
        newFactor = Factor(setOfUnconditioned, factor.conditionedVariables(), factor.variableDomainsDict())
        for oldAssignment in factor.getAllPossibleAssignmentDicts():
            newFactor.setProbability(oldAssignment, newFactor.getProbability(oldAssignment) +
...
def normalize(factor):
    ...
    for assignment in factor.getAllPossibleAssignmentDicts():
        z = z + factor.getProbability(assignment)
    setOfUnconditioned = factor.unconditionedVariables()
    setOfConditioned = factor.conditionedVariables()
    dict = factor.variableDomainsDict()
    for variable in factor.unconditionedVariables():
        if len(dict[variable]) == 1:
            setOfUnconditioned.remove(variable)
            setOfConditioned.append(variable)
    newFactor = Factor(setOfUnconditioned, setOfConditioned,
        factor.variableDomainsDict())
    for assignment in newFactor.getAllPossibleAssignmentDicts():
        newFactor.setProbability(assignment, factor.getProbability(assignment) / z)
    return newFactor
